<ul class="menu col-md-12">
    <li>all</li>
    <li>Front</li>
    <li>Graphisme</li>
    <li>Arts plastiques</li>
</ul>

<div class="portfolio col-sm-12 col-md-12">
    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="createpro.php">
            <div class="projet col-md-12 col-sm-12" id="projet1">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">CREATE PRO</h4>
            <p class="col-md-10 col-md-offset-1">Réalisation de ma toute première page dans le monde du Web. Reprise d'un projet Wordpress à remodeler de A à Z.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5  col-md-3 col-md-offset-1">
        <div class="projet col-md-12 col-sm-12" id="projet2">
            <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
        </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">PASTIME</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <div class="projet col-md-12 col-sm-12" id="projet3">
            <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
        </div>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">C.V</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <div class="projet col-md-12 col-sm-12" id="projet4">
            <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
        </div>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">BOOTSTRAP</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <div class="projet col-md-12 col-sm-12" id="projet5">
            <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
        </div>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">QUIZZ-JS</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <div class="projet col-md-12 col-sm-12" id="projet6">
            <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
        </div>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">AUDIOTHÈME</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet7">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">POMODORO</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet8">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">123 VETO</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet9">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">PONEY CLICKER</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet1">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">EXEMPLE 1</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet2">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">EXEMPLE 2</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet8">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">EXEMPLE 3</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet8">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">EXEMPLE 4</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>

    <div class="polaroid col-xs-12 col-sm-offset-1 col-sm-5 col-md-3">
        <a href="projet1.php">
            <div class="projet col-md-12 col-sm-12" id="projet8">
                <div class="projet-present"> <i class="fa fa-search fa-2x" aria-hidden="true"></i> </div>
            </div>
        </a>
        <div class="description">
            <h4 class="col-md-10 col-md-offset-1">EXEMPLE 5</h4>
            <p class="col-md-10 col-md-offset-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis atque, pariatur repellat maxime eveniet magnam incidunt.</p>
        </div>
    </div>
</div>